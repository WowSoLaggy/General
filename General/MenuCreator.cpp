...
{
  template <typename T>
  std::shared_ptr<T> control(Dx::IControl& i_parent)
  {
  ...
  std::shared_ptr<Dx::Label> label(Dx::IControl& i_parent)
  {
    auto ctrl = control<Dx::Label>(i_parent);
    ctrl->setFont("play.spritefont");
    return ctrl;
  };

  std::shared_ptr<Dx::Button> button(Dx::IControl& i_parent)
  {
    auto ctrl = control<Dx::Button>(i_parent);
    ctrl->setFont("play.spritefont");
  ...
  std::shared_ptr<Dx::Checkbox> checkbox(Dx::IControl& i_parent)
  {
    auto ctrl = control<Dx::Checkbox>(i_parent);
  ...
  std::shared_ptr<Dx::Panel> panel(Dx::IControl& i_parent)
  {
    auto ctrl = control<Dx::Panel>(i_parent);
    return ctrl;
  }
...
void MenuCreator::createMainMenu(Dx::IControl& i_parent) const
{
  ...
  {
    auto versionLabel = label(i_parent);
...
{
  auto debugPanel = panel(i_parent);
  debugPanel->setTexture("gray128.png");
  debugPanel->setPosition({ 16, 16 });

  auto cbShowBb = checkbox(*debugPanel);
  ...
  auto lblShowBb = label(*debugPanel);
